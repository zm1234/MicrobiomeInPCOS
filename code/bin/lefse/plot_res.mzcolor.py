...
colorss_default= ["#E64B35FF","#4DBBD5FF","#00A087FF","#3C5488FF","#F39B7FFF","#8491B4FF","#91D1C2FF","#DC0000FF","#7E6148FF","#B09C85FF"]
def set_plot_colors(colorset):
    if is.none(colorset):
        colorss = colorss_default
    else:
        colorss = colorset
    colors=[]
    for col in colorss:
        tmp=list([int(n, 16) / 255.0 for n in (col[1:3], col[3:5], col[5:7])])
        colors.append(tmp)
    return(colors)
...
def read_params(args):
    ...
    parser.add_argument('--colorset', dest="colorset", type=str, required=False, nargs='+', help="set the color of barplot")
...
def plot_histo_hor(path,params,data,bcl,report_features, colors):
    cls2 = []
    if params['all_feats'] != "":
...
def plot_histo_ver(path,params,data,report_features,colors):
    cls = data['cls']
...
if __name__ == '__main__':
    ...
    for i in params.keys():
         ...
         print(params[i])
    sys.exit()
    colors = setPlotColor(params['colorset'])
    data = read_data(params['input_file'],params['output_file'],params['otu_only'])
    if params['orientation'] == 'v': plot_histo_ver(params['output_file'],params,data,params['report_features'],colors)
    else: plot_histo_hor(params['output_file'],params,data,len(data['cls']) == 2,params['report_features'],colors)
